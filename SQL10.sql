...
--------------ctrl+l посмотреть стоимость запроса(план выполнения)
select * from #example where id between 3000 and 10000 order by id  ---0.0264/0.005
...
create index #example2_nonclu on #example2(id,fild1) --составной
--drop index #example2_nonclu on #example2


select *from #example2 where id>1500 and fild1<5000  --0,0513/0.0513
select * from #example2 order by id, fild1  ---0,666/0.666

select * from #example2 where id=20672 and fild1<5000   ----0,0513/0.00657
...
select fild1 from #example3 where id>2000  --0.0513/0.0328

create index #example3_id_x on #example3(id) include(fild1) --покрытия
...
create index #example4_where on #example4(id) where(id>=1500 and id<25000) --фильтруемый
...
drop index #example5_id on #example5

insert top(10000) #example5(id, fild2) select id,fild2 from #example5
...
create index #example6_id on #example6(id) with(fillfactor = 65) --FILLFACTOR указывает процент заполнения индексных страниц нижнего уровня


insert top(50)percent into #example6(id, fild2) select id,fild2 from #example6
